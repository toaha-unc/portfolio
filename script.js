...
const sampleProjects = [
    ...
    },
    {
        ...
        description: "A comprehensive RESTful API built with Spring Boot, JPA, and PostgreSQL that replicates Twitter's core functionality. Features include user management, tweet creation, following/followers, likes, replies, reposts, hashtags, and mentions.",
        technologies: ["Java", "Spring Boot", "JPA", "PostgreSQL", "Maven", "REST API"],
...
function populateProjects() {
    ...
    sampleProjects.forEach(project => {
        ...
        projectCard.className = 'project-card bg-white rounded-2xl shadow-lg overflow-hidden h-full';
...
function initAnimations() {
    ...
    gsap.fromTo('.flex.flex-col.sm\\:flex-row a', 
...
function initContactForm() {
    ...
    successMessage.className = 'hidden mt-4 p-4 bg-green-600 text-white rounded-lg text-center';
    successMessage.setAttribute('aria-live', 'polite');
    
    const errorMessage = document.createElement('div');
    errorMessage.className = 'hidden mt-4 p-4 bg-red-600 text-white rounded-lg text-center';
    ...
    contactForm.addEventListener('submit', (e) => {
        ...
        emailjs.send('service_bbj9lpo', 'template_pzmadb3', templateParams)
            .then(function(response) {
                console.log('✅ EMAIL SENT SUCCESSFULLY!', response.status, response.text);
                console.log('Response details:', response);
                showSuccess(`Thank you for your message, ${name}! I'll get back to you soon at ${email}.`);
                contactForm.reset();
            }, function(error) {
                console.log('❌ EMAIL SEND FAILED!', error);
                console.log('Error details:', error);
                showError('Sorry, there was an error sending your message. Please try again or contact me directly at toahasiddique.ts@gmail.com');
            })
            .finally(() => {
                // Reset button state
...
function initSparkEffect() {
    document.addEventListener('click', (e) => {
        ...
        for (let i = 0; i < 15; i++) {
            ...
            const hexColor = '#' + Math.floor(r * 255).toString(16).padStart(2, '0') + 
...
function initTypingEffect() {
    ...
    const originalText = bioText.textContent || 'A passionate developer creating amazing digital experiences';
    bioText.textContent = '';
...
function initAboutBackground() {
    ...
    if (!aboutSection || !aboutBackground) {
        console.log('About section elements not found, skipping background animation');
        return;
    }
    
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(75, aboutBackground.clientWidth / aboutBackground.clientHeight, 0.1, 1000);
...
function initCustomCursor() {
    ...
    const interactiveElements = document.querySelectorAll('a, button, .project-card, .skill-badge, input, textarea');
